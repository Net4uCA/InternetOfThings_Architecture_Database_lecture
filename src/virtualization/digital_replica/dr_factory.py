...
class DRFactory:
    ...
    def create_dr(self, dr_type: str, initial_data: Dict[str, Any]) -> Dict:
        ...
        dr = {
            ...
            "metadata": {
                ...
                "privacy_level": initial_data.get('metadata', {}).get('privacy_level', 'private')
            }
        }
    ...
    def _process_section(self, dr: Dict, initial_data: Dict, section_schema: Dict) -> None:
    ...
    def _validate_against_schema(self, dr: Dict, validations: Dict) -> None:
        ...
        if 'required' in validations:
            missing = [field for field in validations['required']
        ...
        if 'properties' in validations:
            for field, rules in validations['properties'].items():
                ...
                if 'required' in rules:
                    ...
                    missing = [subfield for subfield in rules['required']
                               if subfield not in value or value[subfield] is None]
                    if missing:
                        raise ValueError(f"Missing required subfields in {field}: {missing}")
                ...
                if 'properties' in rules:
                    for subfield, subrules in rules['properties'].items():
                        ...
                        # Type validation
                        if 'type' in subrules:
                            self._validate_type(subvalue, subrules['type'], f"{field}.{subfield}")

                        # Range validation
                        if 'minimum' in subrules and subvalue < subrules['minimum']:
                            raise ValueError(f"{field}.{subfield} must be >= {subrules['minimum']}")
                        if 'maximum' in subrules and subvalue > subrules['maximum']:
                            raise ValueError(f"{field}.{subfield} must be <= {subrules['maximum']}")

                        # Enum validation
                        if 'enum' in subrules and subvalue not in subrules['enum']:
                            raise ValueError(f"{field}.{subfield} must be one of {subrules['enum']}")
